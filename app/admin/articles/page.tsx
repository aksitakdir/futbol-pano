...
type TrendItem = { title: string; traffic: string; seoScore: number; source: string; ageHours?: number; link?: string };
...
function IceriklerPageInner() {
  ...
  // trendsSample removed — non-football trends no longer served
  ...
  async function handleFetchTrends(fresh = false) {
    ...
    setTrends([]);
    // trendsSample removed
    try {
      const url = fresh ? "/api/trends?fresh=1" : "/api/trends";
      const res = await fetch(url);
      const data = await res.json();
      setTrends(data.trends ?? []);
      // all_sample removed — multi-source trends only
    } catch {
      setTrends([]);
    }
            ...
            <div className="w-full max-w-lg rounded-xl border border-slate-700/80 bg-slate-900/95 p-6 shadow-2xl">
              ...
              {trendsLoading ? (
                ...
                <>
                  {trends.length > 0 ? (
                    <div className="mb-4">
                      <p className="mb-2 text-[10px] font-semibold uppercase tracking-widest text-emerald-400">Football Trends — Multi-Source (SEO + Freshness)</p>
                      <div className="max-h-[60vh] overflow-y-auto space-y-1.5 pr-1">
                        {trends.map((t, i) => {
                          const srcColor: Record<string, string> = {
                            espn: "text-amber-400", bbc: "text-red-400", guardian: "text-sky-400",
                            sky: "text-blue-400", "google-US": "text-emerald-400", "google-GB": "text-emerald-400",
                          };
                          const srcLabel: Record<string, string> = {
                            espn: "ESPN", bbc: "BBC", guardian: "Guardian",
                            sky: "Sky", "google-US": "Trends US", "google-GB": "Trends UK",
                          };
                          return (
                            <button
                              key={i}
                            ...
                            >
                              ...
                              <div className="flex shrink-0 items-center gap-1.5">
                                <span className={`rounded-full px-1.5 py-0.5 text-[9px] font-bold ${t.seoScore >= 50 ? "bg-emerald-500/20 text-emerald-300" : t.seoScore >= 30 ? "bg-amber-500/20 text-amber-300" : "bg-slate-500/20 text-slate-400"}`}>
                                  {t.seoScore}
                                </span>
                                <span className={`text-[9px] font-semibold ${srcColor[t.source] ?? "text-slate-500"}`}>
                                  {srcLabel[t.source] ?? t.source}
                                </span>
                                {t.ageHours != null && t.ageHours <= 24 && (
                                  <span className="text-[8px] text-emerald-500/70">{t.ageHours}h</span>
                                )}
                              </div>
                            </button>
                          );
