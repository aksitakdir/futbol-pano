      ...
      <div>
        ...
        <div className="flex gap-2 items-stretch">
          <input
            ...
            className="flex-1 rounded-lg border border-slate-700/80 bg-slate-800/70 px-3 py-2 text-sm text-slate-100 placeholder-slate-500 outline-none focus:border-teal-500/60"
          />
          <button
            type="button"
            disabled={uploading}
            onClick={() => fileRef.current?.click()}
            className="shrink-0 rounded-lg border border-teal-500/50 bg-teal-500/10 px-4 py-2 text-[11px] font-bold text-teal-300 uppercase tracking-wider transition hover:bg-teal-500/20 disabled:opacity-50"
          >
        ...
        {uploadError && (
          <p className="mt-1 text-[11px] text-rose-400">{uploadError}</p>
        )}
      </div>
      <div>
        <label className="mb-1 block text-[10px] font-semibold text-slate-400 uppercase tracking-wider">
