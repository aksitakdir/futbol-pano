    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Link href="/admin/yeni" className="rounded-lg bg-emerald-500 px-4 py-2 text-xs font-semibold text-slate-950 transition hover:bg-emerald-400">
            + New Article
          </Link>
        </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
              <Link href="/admin/icerikler" className="flex items-center gap-2 rounded-xl border border-slate-700/60 bg-slate-900/40 px-4 py-3 text-xs font-medium text-slate-300 transition hover:bg-slate-800/30 hover:text-slate-100">
              ...
              <Link href="/admin/ayarlar" className="flex items-center gap-2 rounded-xl border border-slate-700/60 bg-slate-900/40 px-4 py-3 text-xs font-medium text-slate-300 transition hover:bg-slate-800/30 hover:text-slate-100">
            ...
            <div className="overflow-hidden rounded-xl border border-slate-700/60 bg-slate-900/40">
              <div className="flex items-center justify-between border-b border-slate-700/60 px-5 py-3">
                ...
                <Link href="/admin/icerikler" className="text-[11px] font-medium text-emerald-400 transition hover:text-emerald-300">View All →</Link>
              </div>
              {recent.length === 0 ? (
                  ...
                  {recent.map((item) => (
                    <div key={item.id} className="flex items-center gap-3 border-b border-slate-700/40 px-5 py-3 transition hover:bg-slate-800/30 last:border-b-0">
                      <div className="flex-1 min-w-0">
                        <Link href={`/admin/duzenle/${item.id}`} className="block truncate text-sm font-medium text-slate-100 transition hover:text-emerald-300">
