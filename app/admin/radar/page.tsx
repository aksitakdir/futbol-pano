...
export default function AdminRadarPage() {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await supabase.from("contents")
      .select("id, title, title_en, slug, content, status, created_at")
      .eq("category", "radar").order("created_at", { ascending: false });
    ...
    <AdminLayout>
      <div className="mx-auto max-w-4xl">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="text-xl font-bold">Radar Articles</h1>
            <p className="text-xs text-slate-400">Only <strong className="text-slate-300">Published</strong> articles appear on /radar.</p>
        ...
        {loading ? (
          <div className="flex justify-center py-16"><span className="h-4 w-4 animate-spin rounded-full border-2 border-emerald-400 border-t-transparent" /></div>
        ) : rows.length === 0 ? (
          ...
          <div className="space-y-3">
            {rows.map((item) => {
              ...
              return (
                <div key={item.id} className="rounded-xl border border-slate-800/60 bg-slate-900/40 p-4">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0 flex-1">
                      <div className="mb-1 flex flex-wrap items-center gap-2">
                        <span className={`inline-flex rounded-full border px-2 py-0.5 text-[10px] font-semibold ${st.cls}`}>{st.label}</span>
                        <span className="text-[10px] text-slate-500">
                          {new Date(item.created_at).toLocaleDateString("en-US", { day: "numeric", month: "long", year: "numeric" })} · ~{mins} min
                        </span>
                      </div>
                      <h2 className="text-sm font-semibold text-slate-100">{item.title_en || item.title}</h2>
