  ...
  return (
    <div className="flex flex-wrap items-center gap-0.5 rounded-t-xl border border-b-0 border-slate-700/80 bg-slate-900/90 p-1.5">
      ...
      <button
        ...
        className={editor.isActive("heading", { level: 2 }) ? TB_ACTIVE : TB}
        title="Başlık 2"
      >
        H2
      </button>
      <button
        ...
        className={editor.isActive("heading", { level: 3 }) ? TB_ACTIVE : TB}
        title="Başlık 3"
      >
        H3
      </button>
      ...
      <button
        ...
        className={editor.isActive("orderedList") ? TB_ACTIVE : TB}
...
export default function RichTextEditor({ value, onChange, placeholder = "İçerik yazın..." }: Props) {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "min-h-[280px] w-full resize-y rounded-b-xl border border-slate-700/80 bg-slate-900/70 px-4 py-3 text-sm leading-relaxed text-slate-100 outline-none placeholder-slate-500 focus:border-emerald-500/40 [&_h2]:mb-3 [&_h2]:mt-5 [&_h2]:text-lg [&_h2]:font-bold [&_h2]:text-slate-50 [&_h3]:mb-2 [&_h3]:mt-4 [&_h3]:text-base [&_h3]:font-semibold [&_h3]:text-slate-100 [&_p]:mb-4 [&_p]:text-slate-200 [&_ul]:mb-4 [&_ul]:list-disc [&_ul]:pl-5 [&_ul]:text-slate-200 [&_ol]:mb-4 [&_ol]:list-decimal [&_ol]:pl-5 [&_ol]:text-slate-200 [&_li]:mb-1 [&_blockquote]:my-4 [&_blockquote]:border-l-2 [&_blockquote]:border-emerald-500/60 [&_blockquote]:pl-4 [&_blockquote]:italic [&_blockquote]:text-slate-300 [&_a]:text-emerald-300 [&_a]:underline [&_a]:decoration-emerald-500/30 hover:[&_a]:text-emerald-200",
      },
      handleDOMEvents: {
        paste: (view, event) => {
  ...
  // Sync initial value when it changes from parent (e.g. after loading in duzenle)
  useEffect(() => {
    if (!editor) return;
