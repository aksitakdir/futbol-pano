...
async function fetchPlayerStats(name: string) {
  ...
  const { data: two2 } = await supabase
    .from("fc_players")
    .select("overall,pace,shooting,passing,dribbling,defending,physical,photo_url,position,club,league,age")
    .ilike("name", `%${two}%`)
...
export default function EnListelerDetailClient({ slug }: { slug: string }) {
  ...
  if (notFound || !article)
    return (
      <main className="flex min-h-screen flex-col items-center justify-center" style={{ background: "var(--sg-bg)", color: "var(--sg-text-primary)" }}>
        ...
        <Link href="/en/listeler" className="text-sm" style={{ color: "var(--sg-secondary)" }}>← Back to Lists</Link>
      </main>
    );
  ...
  return (
    ...
    >
      {playerCardData && (
        <div className="mb-8">
          <p className="mb-3 text-[11px] font-bold uppercase tracking-[0.2em]" style={{ color: "var(--sg-secondary)", fontFamily: "var(--font-headline)" }}>Player Card</p>
          <PlayerCard
