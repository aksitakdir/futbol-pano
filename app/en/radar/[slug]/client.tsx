...
export default function EnRadarDetailClient({ slug }: { slug: string }) {
  ...
  if (notFound || !article)
    return (
      <main className="flex min-h-screen flex-col items-center justify-center" style={{ background: "var(--sg-bg)", color: "var(--sg-text-primary)" }}>
        ...
        <Link href="/en/radar" className="text-sm" style={{ color: "var(--sg-primary)" }}>← Back to Radar</Link>
      </main>
    );
  ...
  return (
    ...
    >
      {playerCardData && (
        <div className="mb-8">
          <p className="mb-3 text-[11px] font-bold uppercase tracking-[0.2em]" style={{ color: "var(--sg-primary)", fontFamily: "var(--font-headline)" }}>Player Card</p>
          <PlayerCard
