...
function NumberedPlayerEntry({ player, rank, accent }: {
  ...
  return (
    <div style={{
      ...
      <div style={{
        display: "grid",
        gridTemplateColumns: isEven ? "1fr minmax(0, 200px)" : "minmax(0, 200px) 1fr",
        ...
        {isEven ? (
          <>
            ...
            <PlayerCard
              ...
              gLink={`https://www.google.com/search?q=${encodeURIComponent(`${player.name} futbolcu`)}`}
            />
          </>
        ) : (
          <>
        ...
        <div>
          ...
          {content && (
            <div className="article-v2" style={{ fontSize: 18, lineHeight: 1.65, color: "var(--sg-text-secondary)", marginBottom: 56 }}>
              {isHtml ? (
                <ArticleHtmlWithPlayerEmbeds html={content} locale="tr" />
              ) : (
