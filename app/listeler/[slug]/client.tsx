...
const CONTENT_BY_SLUG: Record<string, PageConfig> = {
  "en-iyi-10-genc-stoper": {
    ...
    intro: "2025-26 sezonunda Avrupa genelinde savunma hattını domine eden genç stoperler, sadece fiziksel güçleriyle değil, oyun kurulumuna katkılarıyla da ön plana çıkıyor. Aşağıdaki listede, büyük sahnenin yeni lider adaylarını bulacaksın.",
    sections: [
      { heading: "Modern stoper profilinin bileşenleri", description: "Derine inen pas açıları, geniş alan savunması, geriden oyun kurma kalitesi ve baskı altında karar verme, modern stoperlerin temel bileşenleri. Bu liste, bu metriklerde öne çıkan oyuncuları bir araya getiriyor." },
    ],
    players: [
  ...
  "super-lig-gizli-isimler": {
    ...
    intro: "Süper Lig, yalnızca manşetlere çıkan yıldızlardan ibaret değil. Henüz uluslararası radarın tamamına takılmamış; ancak veri tarafında istikrarlı yükseliş gösteren genç isimler, gelecek sezonların sürpriz transfer hikâyelerini yazmaya aday.",
    sections: [
  ...
  "surpriz-isimler-2025": {
    ...
    intro: "Her sezon beklenmedik çıkışlar hikâyeyi değiştirir. 2025-26 sezonunda da birçok genç oyuncu, ilk 11'de düşünülmezken kilit role evrilerek hem verileri hem de oyunun algısını yukarı taşıdı.",
    sections: [
...
function PlayerCardItem({ player }: { player: Player }) {
  ...
  if (!cardData) {
    return (
      <div className="border p-4" style={{ background: "var(--sg-surface)", borderColor: "rgba(26,58,92,0.5)" }}>
        <p className="text-sm font-semibold" style={{ color: "var(--sg-text-primary)" }}>{player.name}</p>
        <p className="mt-1 text-xs" style={{ color: "var(--sg-text-muted)" }}>{player.club} · {player.age} yaş</p>
        <p className="mt-2 text-xs" style={{ color: "var(--sg-text-secondary)" }}>{player.strengths}</p>
      </div>
    );
  }
  ...
  return (
    <section className="mb-8">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-[11px] font-bold uppercase tracking-[0.22em]" style={{ color: "var(--sg-secondary)" }}>Oyuncu Kartları</h2>
        <span className="text-[11px]" style={{ color: "var(--sg-text-muted)" }}>{players.length} oyuncu</span>
      </div>
      <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
        {players.map((p) => <PlayerCardItem key={p.name} player={p} />)}
      </div>
    </section>
  );
}
...
export default function ListDetailClient({ slug }: { slug: string }) {
    ...
    return (
      <main className="flex min-h-screen items-center justify-center" style={{ background: "var(--sg-bg)" }}>
        <span className="h-5 w-5 animate-spin rounded-full border-2 border-t-transparent"
          style={{ borderColor: "var(--sg-secondary)", borderTopColor: "transparent" }} />
      </main>
    );
  }
    ...
    return (
      ...
      >
        {(() => {
          ...
          let players: Array<{ name: string; overall: number; position: string; club: string; photo_url?: string; scout_note: string }> = [];
          try { players = JSON.parse(dbContent.players_json); } catch { return null; }
          if (!players.length) return null;
          return (
            <section className="mb-8">
              <div className="mb-4 flex items-center justify-between">
                <p className="text-[11px] font-bold uppercase tracking-[0.2em]" style={{ color: "var(--sg-secondary)" }}>Oyuncu Listesi</p>
                <span className="text-[11px]" style={{ color: "var(--sg-text-muted)" }}>{players.length} oyuncu</span>
              </div>
              <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
                {players.map((p) => (
                  <PlayerCardItem key={p.name} player={{ name: p.name, club: p.club, age: 0, position: p.position, strengths: p.scout_note ?? "" }} />
                ))}
              </div>
            </section>
          );
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center" style={{ background: "var(--sg-bg)", color: "var(--sg-text-primary)" }}>
      ...
      <p className="mb-6 text-sm" style={{ color: "var(--sg-text-muted)" }}>Bu liste bulunamadı veya henüz yayınlanmadı.</p>
      <Link href="/listeler" className="text-sm" style={{ color: "var(--sg-secondary)" }}>← Listelere Dön</Link>
    </main>
  );
}
