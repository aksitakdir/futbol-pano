...
export default function RadarDetailClient({ slug }: { slug: string }) {
  ...
  if (notFound || !article) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center" style={{ background: "var(--sg-bg)", color: "var(--sg-text-primary)" }}>
        ...
        <p className="mb-6 text-sm" style={{ color: "var(--sg-text-muted)" }}>Bu radar yazısı bulunamadı veya henüz yayınlanmadı.</p>
        <Link href="/radar" className="text-sm" style={{ color: "var(--sg-primary)" }}>← Radar&apos;a Dön</Link>
      </main>
    );
  }
  ...
  return (
    ...
    >
      {playerCardData ? (
        <div className="mb-8">
          <p className="mb-3 text-[11px] font-bold uppercase tracking-[0.2em]" style={{ color: "var(--sg-primary)" }}>Oyuncu Kartı</p>
          <PlayerCard
